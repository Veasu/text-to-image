...
const createTextData = (
  ...
  for (let n = 0; n < wordCount; n += 1) {
    ...
    fontHeight =
      textContext.measureText('M').actualBoundingBoxAscent +
  ...
  const textHeight = textY + fontHeight;

  //let actualHeight = metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent;

  console.log(fontHeight * lines);
  console.log(textHeight);
  return textContext.getImageData(0, 0, maxWidth, textHeight + fontHeight);
};
...
const createCanvas = (content, conf) => {
  ...
  const canvas = Canvas.createCanvas(
    conf.maxWidth,
    conf.customHeight + conf.margin * 2,
  );
  ...
  ctx.putImageData(
    textData,
    canvas.width / 2 - textData.width / 2,
    canvas.height / 2 - textData.height / 2,
  );
